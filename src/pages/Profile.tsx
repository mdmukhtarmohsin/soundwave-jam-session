...
const Profile = () => {
  ...
  useEffect(() => {
    // Load profile data whenever the component mounts
    console.log(
      "[Profile Page] useEffect running, calling loadProfileAndStats..."
    );
    loadProfileAndStats();
  }, []); // <-- Changed dependency array from [user] to []

  const loadProfileAndStats = async () => {
    console.log("[Profile Page] loadProfileAndStats called.");
    if (!user) {
      console.log(
        "[Profile Page] No user found in loadProfileAndStats, returning."
      );
      // Optionally set loading to false or handle empty state
      // setIsLoading(false);
      return;
    }

    console.log("[Profile Page] User found, proceeding to load data.");
    setIsLoading(true);
    ...
    try {
      ...
      const myTracksPromises = jamRooms.map((room) =>
        getTracksByJamRoomId(room.id)
      );
      ...
      if (roomsHosted > 0) {
        avgLoopsPerSession =
          Math.round((loopsRecorded / roomsHosted) * 10) / 10; // Round to 1 decimal
      }
      ...
      console.log("[Profile Page] loadProfileAndStats finished.");
      setIsLoading(false);
    }
  };
              ...
              <TabsContent value="stats">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <Card className="bg-secondary border-white/10">
                    <CardHeader className="pb-2 pt-4">
                      <div className="w-8 h-8 rounded-lg bg-soundboard-primary/20 flex items-center justify-center">
                        <Download
                          size={16}
                          className="text-soundboard-primary"
                        />
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold">
                        {stats.mixdownsExported}
                      </div>
                  ...
                  <Card className="bg-secondary border-white/10">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        {stats.avgLoopsPerSession}
                      </div>
                      <p className="text-sm text-white/60">
                        Avg. Loops per Session
                      </p>
                    </CardContent>
                  </Card>
                </div>
                ...
                {stats.recentActivity.length > 0 ? (
                  ...
                  <div className="glass-morphism p-8 rounded-lg flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm text-white/40">
                      Create jam rooms or record loops to see your activity
                    </p>
                  </div>
                )}
              </TabsContent>
