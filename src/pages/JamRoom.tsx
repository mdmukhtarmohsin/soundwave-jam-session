...
const JamRoom = () => {
  ...
  const { user } = useAuth();
  const { getJamRoomById, updateJamRoom, getTracksByJamRoomId, uploadTrack } = useSupabase();
  ...
  const loadTracks = async () => {
    ...
    try {
      ...
      const uniqueCreators = new Map<string, { name: string; isHost: boolean }>();
      ...
      const updatedCollaborators = Array.from(uniqueCreators.entries()).map(([id, info]) => ({
        id,
        name: info.name,
        avatar: '',
        isHost: info.isHost,
    ...
    return (
      <div className="min-h-screen bg-soundboard-dark text-white flex items-center justify-center">
        <div className="flex flex-col items-center">
          <WaveformVisualizer isAnimated={true} height="h-12" className="w-48" />
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Input
                ...
                disabled={!isHost}
                className={`text-2xl font-bold bg-transparent border-transparent hover:border-white/10 focus:border-white/20 focus:bg-white/5 p-1 ${!isHost ? 'cursor-default' : ''}`}
              />
              {isHost && (
                <Button
                  variant="outline"
              ...
              <div className="glass-morphism rounded-xl p-6 mt-6">
                ...
                <div className="flex flex-col items-center">
                  ...
                  >
                    ...
                    <span>{isPlaying ? 'Pause All Tracks' : 'Play All Tracks'}</span>
                  </Button>
                </div>
              </div>
            )}
